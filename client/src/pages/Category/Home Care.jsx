...
const HomeCare = () => {
  ...
  const fetchProducts = async () => {
    setLoading(true)
    try {
      const result = await productAPI.getProducts(null, 'Home Care')
      if (result.ok) {
    ...
    <div className="min-h-screen bg-[#F8FAFC] text-slate-900 selection:bg-[#3FB893] selection:text-white">
      ...
      <section className="relative mt-16 sm:mt-20 overflow-hidden border-b border-gray-200 bg-white">
        <div
          ...
          style={{
            backgroundImage: `url(${homeCareImage})`,
        ...
        <div className="max-w-7xl mx-auto px-6 sm:px-8 pb-20 relative z-10">
          <div className="flex items-center gap-2 text-[11px] mt-5 sm:mt-20 sm:mb-50 sm:text-[15px] font-mono uppercase tracking-[0.2em] text-white mb-6">
            ...
            <span>/</span>
            <span className="text-white font-semibold">Home Care</span>
          </div>
          ...
          <div className="max-w-3xl">
            ...
            >
              Home Care <span className="text-red-600">Packaging.</span>
            </h1>
            <p className="text-sm sm:text-lg text-white font-medium leading-relaxed max-w-2xl">
              Durable and functional packaging for cleaning products, detergents, and household chemicals.
            </p>
          </div>
        </div>
      </section>
              ...
              {loading ? (
                ...
                <div className="text-center py-24 border border-[#DEDDD6] dark:border-[#2A2D32] bg-white dark:bg-[#1C1F23] max-w-md mx-auto">
                  ...
                  <p className="text-[#5C6066] dark:text-[#9B9D9F] text-xs px-8 leading-relaxed max-w-xs mx-auto">
                    No active home care metrics matched this profile configuration query. Select alternate configurations.
                  </p>
                </div>
              )}
            </main>
