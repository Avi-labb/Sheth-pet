...
import foodBeverageImage from '../../assets/images/Food & Beverages.png'
...
const FoodBeverages = () => {
 ...
 setLoading(true)
 try {
 const result = await productAPI.getProducts(null, 'Food & Beverages')
 if (result.ok) {
 ...
 style={{
 backgroundImage: `url(${foodBeverageImage})`,
 ...
 <span>/</span>
 <span className="text-white font-semibold">Food & Beverages</span>
 </div>
 ...
 >
 Food & Beverages <span className="text-red-600">Packaging.</span>
 </h1>
 <p className="text-sm sm:text-lg text-white font-medium leading-relaxed max-w-2xl">
 Food-safe packaging solutions for beverages, condiments, and food products. Ensures freshness and meets all food safety regulations.
 </p>
 </div>
 </div>
 </section>
 ...
 <motion.div
 key={product._id}
 className="group flex flex-col justify-between bg-[#FAFAF8] hover:bg-white transition-colors relative border border-[#DEDDD6] cursor-pointer"
 initial={{ opacity: 0 }}
 ...
 >
 <div className="flex-1 flex flex-col" style={{ pointerEvents: 'none' }}>
 ...
 <div className="flex flex-wrap gap-2 sm:gap-1" style={{ pointerEvents: 'auto' }}>
 ...
 <div className="px-4 pb-4 grid grid-cols-2 gap-2" style={{ pointerEvents: 'auto' }}>
 <Link
 to={`/innovate`}
 ...
 No active food & beverage metrics matched this profile configuration query. Select alternate configurations.
 </p>
 </div>
 )}
 </main>
