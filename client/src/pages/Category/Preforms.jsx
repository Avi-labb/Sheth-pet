...
const Container = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      setLoading(true)
      try {
        const result = await productAPI.getProducts('Preforms')
        if (result.ok) {
          setProducts(result.data.products)
        }
      } catch (error) {
        console.error("Error fetching preforms:", error)
      }
      setLoading(false)
    }
      ...
      <section className="bg-gradient-to-br from-slate-900 to-slate-800 py-20 px-6 text-center">
        ...
        >
          ...
          <p className="text-lg md:text-xl text-white/80 font-normal" style={{ fontFamily: "'Inter', sans-serif" }}>
            Our complete range of preform solutions
          </p>
        </motion.div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-6">
          {loading ? (
            ...
            <div className="text-center py-20">
              ...
              <h3 className="text-xl font-semibold text-slate-400 mb-2">No containers available yet</h3>
              <p className="text-slate-500">Check back soon for our complete range of containers</p>
            </div>
          )}
        </div>
      </section>
                      ...
                      {selectedProductDetail.length && (
                        <div className="p-5 bg-slate-900 border border-slate-800 rounded-xl hover:border-slate-700 transition-colors">
                          <div className="flex items-center gap-3 mb-2">
                            ...
                            <p className="text-xs font-bold text-slate-500 uppercase tracking-widest">Length</p>
                          </div>
                          <p className="text-slate-200 font-medium">{selectedProductDetail.length}</p>
                        </div>
                      )}
