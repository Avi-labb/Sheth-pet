...
const Jar = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      setLoading(true)
      try {
        const result = await productAPI.getProducts('Jars')
        if (result.ok) {
      ...
      <section className="bg-gradient-to-br from-slate-900 to-slate-800 py-20 px-6 text-center">
        ...
        >
          ...
          <p className="text-lg md:text-xl text-white/80 font-normal" style={{ fontFamily: "'Inter', sans-serif" }}>
            Our complete range of PET jar solutions
          </p>
        </motion.div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-6">
          {loading ? (
            ...
            <div className="text-center py-20">
              ...
              <h3 className="text-xl font-semibold text-slate-400 mb-2">No jars available yet</h3>
              <p className="text-slate-500">Check back soon for our complete range of jars</p>
            </div>
          )}
        </div>
      </section>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      ...
                      {selectedProductDetail.volume && (
                        <div className="p-5 bg-slate-900 border border-slate-800 rounded-xl hover:border-slate-700 transition-colors">
                          ...
                          <p className="text-slate-200 font-medium">{selectedProductDetail.volume}</p>
                        </div>
                      )}
                      ...
                      {selectedProductDetail.diameter && (
                        <div className="p-5 bg-slate-900 border border-slate-800 rounded-xl hover:border-slate-700 transition-colors">
                          <div className="flex items-center gap-3 mb-2">
                            <div className="w-8 h-8 rounded-lg bg-slate-800 flex items-center justify-center">
                              <Box size={14} className="text-slate-400" />
                            </div>
                            <p className="text-xs font-bold text-slate-500 uppercase tracking-widest">Diameter</p>
                          </div>
                          <p className="text-slate-200 font-medium">{selectedProductDetail.diameter}</p>
                        </div>
                      )}
