...
const ProductDetail = () => {
 ...
 <div className="h-3 bg-[#DEDDD6] w-1/6" />
 <div className="flex flex-col lg:flex-row gap-10">
 <div className="w-full lg:w-[42%] aspect-square bg-[#DEDDD6]" />
 <div className="w-full lg:w-[58%] space-y-6">
 <div className="h-9 bg-[#DEDDD6] w-3/4" />
 <div className="h-3 bg-[#DEDDD6] w-1/4" />
 <div className="h-32 bg-[#DEDDD6]" />
 <div className="h-11 bg-[#DEDDD6] w-1/2" />
 </div>
 </div>
 </div>
 </div>
 ...
 if (!product) {
 return (
 <div className="min-h-screen bg-[#FAFAF8] flex items-center justify-center pt-24">
 <div className="max-w-md w-full mx-auto px-8 py-12 text-center bg-white border border-[#DEDDD6]">
 ...
 <h1 className="text-xl font-bold text-[#15171A] mb-2" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>
 Component not found
 </h1>
 <p className="text-[#5C6066] mb-8 text-sm leading-relaxed">
 We couldn't locate this part in the catalog. It may have been discontinued or the link is out of date.
 </p>
 <Link
 to="/products"
 className="inline-flex items-center gap-2 px-5 py-2.5 bg-[#15171A] text-white text-xs font-mono uppercase tracking-wider transition-opacity hover:opacity-85"
 >
 <ArrowLeft size={14} />
 Back to catalog
 </Link>
 ...
 // Helper to get effective spec value: color-specific override, or fall back to product default
 const getEffectiveSpec = (fieldName) => {
 if (selectedColor && product.colorSpecs && product.colorSpecs[selectedColor]) {
 const colorSpec = product.colorSpecs[selectedColor]
 ...
 const effectiveVolume = getEffectiveSpec('volume')
 const effectiveNeckSize = getEffectiveSpec('neckSize')
 const effectiveNeckProfile = getEffectiveSpec('neckProfile')
 const effectivePilfer = getEffectiveSpec('pilfer')
 const effectiveOFC = getEffectiveSpec('ofc')
 const effectiveHeight = getEffectiveSpec('height')
 ...
 const specRows = [
 { label: 'Volume', value: formatSpecValue(effectiveVolume) },
 { label: 'Neck size', value: formatSpecValue(effectiveNeckSize) },
 { label: 'Neck profile', value: formatSpecValue(effectiveNeckProfile) },
 { label: 'Pilfer proof', value: formatSpecValue(effectivePilfer) },
 { label: 'OFC value', value: formatSpecValue(effectiveOFC) },
 { label: 'Height', value: formatSpecValue(effectiveHeight) },
 ...
 return (
 <div
 className="min-h-screen bg-[#FAFAF8] text-[#15171A] pt-28 pb-24 selection:bg-[#D4530F] selection:text-white"
 style={{ fontFamily: "'Inter', system-ui, sans-serif" }}
 >
 <div className="max-w-9xl mx-auto px-4 sm:px-6 lg:px-8">

 {/* Breadcrumb */}
 <nav className="flex items-center gap-2 text-[11px] font-mono uppercase tracking-wider text-[#8C8E8A] mb-10 pb-4 border-b border-[#DEDDD6]">
 ...
 <span className="opacity-40">/</span>
 <span className="text-[#15171A] truncate max-w-[180px] sm:max-w-xs">{product.name}</span>
 </nav>
 ...
 <div className="flex border border-[#DEDDD6] bg-white">

 <div className="w-9 shrink-0 bg-[#15171A] flex items-center justify-center py-4">
 <span
 className="font-mono text-[10px] tracking-[0.25em] uppercase text-[#FAFAF8] whitespace-nowrap"
 ...
 <div className="text-[#C8C6BD] flex flex-col items-center gap-3">
 ...
 <span className="inline-flex items-center px-2.5 py-1 bg-[#FAFAF8] border border-[#DEDDD6] text-[#15171A] text-[10px] font-mono uppercase tracking-widest">
 {product.category}
 </span>
 </div>
 )}
 ...
 <div className="absolute bottom-4 right-4 flex items-center gap-1.5 px-2.5 py-1.5 bg-[#FAFAF8]/90 border border-[#DEDDD6] text-[#5C6066]">
 ...
 className={`relative w-14 h-14 shrink-0 border bg-white overflow-hidden transition-colors ${
 isActive
 ? 'border-[#D4530F]'
 : 'border-[#DEDDD6] hover:border-[#8C8E8A]'
 }`}
 ...
 {/* Certification strip */}
 <div className="grid grid-cols-3 border border-[#DEDDD6] mt-3">
 {[
 ...
 <div
 key={label}
 className={`flex flex-col items-center text-center gap-1.5 py-3 ${i !== 2 ? 'border-r border-[#DEDDD6]' : ''}`}
 >
 ...
 {currentMoq && (
 <div className="flex items-center justify-between border border-gray-400 px-5 py-4">
 ...
 <tr key={index} className="border-b border-[#DEDDD6] last:border-b-0">
 <td className="py-2.5 text-sm text-[#5C6066]">{row.label}</td>
 ...
 <div className="border-t border-[#DEDDD6] pt-5 space-y-3">
 ...
 <span key={idx} className="text-sm text-[#3D4044]">
 ...
 <Link
 to="/products"
 className="py-3.5 px-5 border border-[#DEDDD6] text-[#5C6066] text-xs font-mono uppercase tracking-wider text-center hover:border-[#8C8E8A] transition-colors"
 >
 Back to catalog
 </Link>
 </div>
 ...
 <div className="mt-24 border-t border-[#DEDDD6] pt-12">
 <h2
 ...
 className="group flex flex-col transition-colors border border-[#DEDDD6] gap-1"
 >
 <div className="relative aspect-[4/4] flex items-center justify-center overflow-hidden border border-[#DEDDD6] gap-1">
 {relatedImage ? (
 <img
 src={relatedImage}
 ...
 <Package size={22} strokeWidth={1.5} className="text-[#C8C6BD]" />
 )}
 </div>

 <div className="p-3.5 flex items-start justify-between gap-2">
 <div className="min-w-0">
 <h3 className="text-xs font-medium text-[#15171A] truncate group-hover:text-[#D4530F] transition-colors">
 {relatedProduct.name}
 </h3>
 {relatedProduct.sku && (
